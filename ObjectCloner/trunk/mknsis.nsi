...
Name "${PROGRAM_NAME}"
InstallDir "$PROGRAMFILES\s3pi ObjectCloner"
...
gotAll:  
  ...
  !include ${INSTFILES}
;  File /a *.dll *.txt ObjectCloner.exe gpl-3.0.txt ${tla}-Version.txt

  StrCmp "Y" $wantSM wantSM noWantSM
wantSM:
  CreateDirectory "$SMPROGRAMS\${tla}"
  CreateShortCut "$SMPROGRAMS\${tla}\${tla}.lnk" "$INSTDIR\ObjectCloner.exe" "" "" "" SW_SHOWNORMAL "" "${PROGRAM_NAME}"
...
Function CheckInUse
  ...
  IfFileExists "$INSTDIR\ObjectCloner.exe" Exists
  Return
Exists:
  ClearErrors
  FileOpen $0 "$INSTDIR\ObjectCloner.exe" a
  IfErrors InUse
  FileClose $0
  Return
InUse:
